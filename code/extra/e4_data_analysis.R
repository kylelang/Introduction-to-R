### Title:    Introduction to R Extra 4: Data Analyses
### Author:   Kyle M. Lang
### Created:  2016-01-27
### Modified: 2025-01-16
...
tapply(bfi$age, bfi$edu, median)
with(bfi, tapply(age, edu, median))
bfi %$% tapply(age, edu, median)
...
aggregate(bfi[scaleNames], bfi[c("sex", "edu")], mean)
...
bfi %>%
    group_by(sex) %>%
    summarise(across(everything(), .fns = class))
...
(out <- bfi %$% table(sex, edu) %>% chisq.test())
...
fit2 <- lm(neuro ~ age + sex + agree + extra + open + consc, data = bfi)
summary(fit2)
